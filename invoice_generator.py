class invoice_user_interface():
    def invoice_header(self, name: str, invoice_num, ):
        space = " "
        invoice_num_len = len(str(invoice_num))
        space_num = 56 - len(name) - invoice_num_len
        return f"-" * 67 + f"\n{name}{space * space_num}Invoice N. {invoice_num}"

    def invoice_print_line(self, job, quantity, rate, amount):
        space = " "
        space1 = 43 - len(job) - len(str(quantity))
        space2 = 9 - len(str(rate))
        space3 = 12 - len(str(amount))
        return f"{job}{space * space1}{quantity}{space * space2}{rate} Tk.{space * space3}{amount} Tk."

    def invoice_total(self, t):
        space = " "
        spaces = 59 - len(str(t))
        return f"{space * spaces}TOTAL: {t} Tk."
    ...
    def output(self):
        # header
        print(self.invoice_header(self.invoice_from, self.invoice_number))
        ...
        for i in range(len(self.lines)):
            print(self.invoice_print_line(self.lines[i][0], self.lines[i][1], self.lines[i][2], self.lines[i][3]))
            print("-" * 68)
        # total
        print(self.invoice_total(self.total))
        print("\n\n\n")
        print(f"Notes:\n{self.notes}")
        print("-" * 68)
        print("\n\n\n\n")
...
class InvoiceGenerator(invoice_user_interface):
    def __init__(self):
        self.total = 0
        self.lines = []
...
Made by Shakil Ahmed - 2022 <ahmedmshakil1@gmail.com>
'''
